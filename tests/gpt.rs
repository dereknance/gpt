...
fn test_create_simple_on_device() {
    ...
    gdisk
        .add_partition("test1", 1024 * 12, gpt::partition_types::BASIC, 0)
        .unwrap();
    gdisk
        .add_partition("test2", 1024 * 18, gpt::partition_types::LINUX_FS, 0)
        .unwrap();
    let mut mem_device = gdisk.write().unwrap();
...
fn test_helper_gptdisk_write_efi_unused_partition_entries(lb_size: disk::LogicalBlockSize) {
    ...
    gdisk
        .add_partition("test1", part1_bytes, gpt::partition_types::BASIC, 0)
        .unwrap();
    gdisk
        .add_partition(
            "test2",
